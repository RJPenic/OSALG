...
namespace OSALG_vector {
	...
	void compute_vector(char const *reference_safe, char const *query_safe, int ref_len,
					__m128i const &match_vec, __m128i const &mismatch_vec, __m128i const &go1_vec, __m128i const &ge1_vec, __m128i const &param_sum_vec,
					int8_t **u, int8_t **v, int8_t **x, int8_t **y, int i, int j) {
		int m = i - j;
		int n = j;

		//z[i, j]
		__m128i reference_chars = _mm_loadu_si128((__m128i *)&reference_safe[ref_len - m]);//already reversed
		__m128i query_chars = _mm_loadu_si128((__m128i *)&query_safe[n - 1]);//load chars
		
		__m128i mask = _mm_cmpeq_epi8(reference_chars, query_chars);
		__m128i z_vec = _mm_blendv_epi8 (mismatch_vec, match_vec, mask);
		z_vec = _mm_add_epi8(z_vec, param_sum_vec);	

		__m128i u_prev_vec = _mm_loadu_si128((__m128i *)&u[i - 1][j - 1]); // u[i, j - 1]
		__m128i v_prev_vec = _mm_loadu_si128((__m128i *)&v[i - 1][j]); // v[i - 1, j]
		__m128i x_prev_vec = _mm_loadu_si128((__m128i *)&x[i - 1][j]); // x[i - 1, j]
		__m128i y_prev_vec = _mm_loadu_si128((__m128i *)&y[i - 1][j - 1]); // y[i, j - 1]

		__m128i temp =  _mm_add_epi8(x_prev_vec, v_prev_vec);
		z_vec = _mm_max_epi8 (z_vec, temp);

		temp =  _mm_add_epi8(y_prev_vec, u_prev_vec);
		z_vec = _mm_max_epi8 (z_vec, temp);

		//u[i, j] and v[i, j]
		__m128i u_vec = _mm_sub_epi8(z_vec, v_prev_vec);
		__m128i v_vec = _mm_sub_epi8(z_vec, u_prev_vec);

		_mm_storeu_si128((__m128i *)&u[i][j], u_vec);
		_mm_storeu_si128((__m128i *)&v[i][j], v_vec);
		
		//x[i, j]
		temp = _mm_sub_epi8(x_prev_vec, u_vec);
		temp = _mm_add_epi8(temp, go1_vec);
		__m128i x_vec = _mm_max_epi8(zero_vec, temp);

		_mm_storeu_si128((__m128i *)&x[i][j], x_vec);

		//y[i, j]
		temp = _mm_sub_epi8(y_prev_vec, v_vec);
		temp = _mm_add_epi8(temp, go1_vec);
		__m128i y_vec = _mm_max_epi8(zero_vec, temp);

		_mm_storeu_si128((__m128i *)&y[i][j], y_vec);
	...
	void long_gaps_alignment(std::string const &reference, std::string const &query, std::string &cigar, bool extended_cigar) {
		...
		//ge ==> gap extend
		//go ==> gap open
		__m128i ge1_vec = _mm_set1_epi8(ge[0]);
		__m128i go1_vec = _mm_set1_epi8(go[0]);
		__m128i param_sum_vec = _mm_set1_epi8(2 * (go[0] + ge[0]));
		
		__m128i match_vec = _mm_set1_epi8(MATCH_SCORE);
		__m128i mismatch_vec = _mm_set1_epi8(MISMATCH_SCORE);
